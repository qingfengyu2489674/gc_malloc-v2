...
TEST(ManagedListTest, ReclaimRemovesFreedNodesSinglePass) {
    ...
    for (auto* b : blocks) ml.attach_used(b);
    ...
    // 第一次应回收 blocks[1]
    BlockHeader* r1 = ml.reclaim_next();
    ...
    // 第二次应回收 blocks[2]
    BlockHeader* r2 = ml.reclaim_next();
    ...
    // 第三次应无可回收
    BlockHeader* r3 = ml.reclaim_next();
